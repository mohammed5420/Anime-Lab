...
const AnimeCard = ({ anime, type }) => {
  const randomColor = ["gray", "indigo", "red", "blue","yellow","green","purple","pink"];
  ...
  return (
    ...
    {/* hello */}
      ...
      <div className="absolute bottom-0 left-0 right-0 h-24 pt-6 bg-gradient-to-t from-gray-900  to-transparent">
        <h3 className="text-md overflow-truncate h-5 overflow-hidden text-white font-bold my-2 px-5 capitalize">{`${
          anime.name ? anime.name : ""
        } ${anime.title ? anime.title : ""}`}</h3>
        {anime.score && (
          <p className="text-gray-200 text-md px-5 mb">
            <img className="w-5 h-5 inline" src={star} alt="star" />{" "}
